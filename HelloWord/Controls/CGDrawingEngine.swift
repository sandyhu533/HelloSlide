...
class StrokeCGView: UIView {
    ...
    //displayOptions代表着绘图的模式，模式一旦改变就需要重画整个view。且当不是debug模式时，dirtyRectViews要隐藏。
    var displayOptions = StrokeViewDisplayOptions.highlight {
        didSet {
...
private extension StrokeCGView {
    ...
    func draw(segment: StrokeSegment,
        ...
        if displayOptions == .highlight {
            ...
            drawHighlight(in: context, toSample: toSample, fromSample: fromSample, forceAccessBlock: forceAccessBlock)
    ...
    /// Renders the stroke in a highlight-like style.
    /// - Tag: drawCalligraphy
    func drawHighlight(in context: CGContext,
                       toSample: StrokeSample,
                       fromSample: StrokeSample,
                       forceAccessBlock: (_ sample: StrokeSample) -> CGFloat) {
        
        let color = UIColor(netHex: 0x1296db, alpha: 0.25)
        context.setFillColor(color.cgColor)
        
        context.beginPath()
        context.addLines(between: [
            fromSample.location + CGVector(dx: 0, dy: 10.0),
            toSample.location + CGVector(dx: 0, dy: 10.0),
            toSample.location - CGVector(dx: 0, dy: 10.0),
            fromSample.location - CGVector(dx: 0, dy: 10.0),
            ])
        context.closePath()
